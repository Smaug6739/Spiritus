...
class default_1 extends CommandClass_1.default {
    ...
    execute(interaction, args, settings) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let user;
            if (args.get('user')) {
                user = yield this.spiritus.util.resolveMember(interaction.guild, args.get('user').value);
                user = user.user;
                if (!user)
                    return interaction.replyErrorMessage('User not found.');
                dbUser = yield this.spiritus.db.getUser(interaction.guild.id, user.id);
                if (!dbUser)
                    return interaction.replyErrorMessage('User not found.');
            }
            else {
                dbUser = yield this.spiritus.db.getUser(interaction.guild.id, interaction.user.id);
                if (!dbUser)
                    return interaction.replyErrorMessage('User not found');
