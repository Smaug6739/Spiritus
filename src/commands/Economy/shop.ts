...
export default class extends Command {
  ...
  async execute(
    ...
    if (!args.first()) {
      ...
      if (settings.shop) {
        settings.shop.slice(0, 20).forEach((objet) => {
          haveItems = true;
          embed.addField(
            `${this.emojis.arrow} ${objet.name} __Price :__ ${objet.price} ${this.emojis.coins}`,
            `__Description :__ ${objet.description}`
          );
        });
        if (!haveItems)
        ...
        embed.addField(
          `\u200b`,
          `You can buy object with the command \`${settings.prefix}shop buy <item_name>\``,
          false
        );
      } else {
    ...
    if (args.get("buy")) {
      const dbUser = await this.util.getUser(
        interaction.user,
        interaction.guild!.id
      );
      if (!dbUser)
        return interaction.replyErrorMessage(
          `You have 0${this.emojis.coins} so you can't buy object in shop.`
        );
      ...
      if (!existObj) return interaction.replyErrorMessage(`Object not found.`);
        ...
        this.db.updateUser(interaction.member, {
          coins: remCoins,
          objets: arr,
        });
        interaction.replySuccessMessage(
          `You just bought  \`${existObj.name}\`. This object is now in your inventory.`
        );
      }
    }
  }
