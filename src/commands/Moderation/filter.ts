  ...
  async execute(
    ...
    switch (interaction.subcommand) {
      case "add":
        ...
        await this.spiritus.db.updateGuild(interaction.guildId!, {
          $push: { filter: wordToAdd },
        });
        interaction.replySuccessMessage(
          `This word is now forbidden on the server.`
        );
        break;
      case "rem":
        ...
        await this.spiritus.db.updateGuild(interaction.guildId!, {
          $pull: { filter: wordToRemove },
        });
        interaction.replySuccessMessage(
          `The word \`${wordToRemove}\` is now allowed.`
        );
        break;
      ...
      case "list":
        if (!settings.filter || settings.filter.length < 1)
          return interaction.replyErrorMessage(
            `There are no forbidden words for this server. To add it use the command \`${settings.prefix}filter add <mot>\``
          );
        const embed = new MessageEmbed()
          .setTitle(
            `List of words in blacklist of this guild**${
              interaction.guild!.name
            }** | ${settings.filter.length} total`
          )
          .setThumbnail(
