...
export default class {
  ...
  async run(interaction: ICommandInteraction) {
    ...
    if (command.userPermissions.includes("MODERATOR")) {
      const isMod = await this.spiritus.functions.checkMod(
        interaction.member,
        settings
      );
      if (!isMod)
    ...
    if (
      ...
      !this.spiritus.admins.includes(interaction.user.id)
    ) {
      return interaction.replyErrorMessage(
        `You don't have permissions for use this command.`
      );
        ...
        if (!interaction.guild!.me!.permissions.has(permission))
          return interaction.replyErrorMessage(
            `I need the \`${command.botPermissions
              ...
              .join(", ")}\` permission(s) for this command.`
          );
      }
    }
    /* ---------------COOLDOWNS--------------- */
        ...
        if (timeNow < cdExpirationTime) {
          ...
          return interaction.replyErrorMessage(
            `Please wait ${timeLeft.toFixed(
              0
            )} second(s) before using the command \`${
              command.help.name
            }\` again .`
          );
        }
