...
export default class {
	...
	async run(message: Message) {
		if (message.author.bot || message.author.system || !message.guild || !message.member) return;
		if (!(message.channel as TextChannel | ThreadChannel | NewsChannel).permissionsFor(message.guild!.me!).has('SEND_MESSAGES')) return;
		...
		if (settings.expsysteme) {
			if (!dbUser) await this.spiritus.db.createUser({
				...
				if (expCd >= 10 && expCd <= 15) {
					...
					if (dbUser.level < userLevel) {
						if (settings.salonranks != "") {
							const channel = message.guild!.channels.cache.get(settings.salonranks) as TextChannel | ThreadChannel;
