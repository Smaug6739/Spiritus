...
module.exports.run = async (client, message, args, settings) => {
    ...
    if (args[0].toLowerCase() === 'add') {
        if (!message.member.permissions.has('ADMINISTRATOR')) return message.channel.sendErrorMessage(`Vous devez avoir la permission d'admin pour utiliser cette commande.`);
        const role = client.resolveRole(message.guild, args[1]);
        if (!role) return message.channel.sendErrorMessage(`Je n'ai pas trouver ce role`);
        if (settings.modRoles.includes(role.id)) return message.channel.sendErrorMessage(`Ce role est déja modérateur sur le serveur.`);
        else {
            settings.modRoles.push(role.id);
            await settings.save();
            return message.channel.sendSuccessMessage(`Ce role est maintanant modérateur.`);
        }
    }
    if (args[0].toLowerCase() === 'rem') {
        ...
        const role = client.resolveRole(message.guild, args[1]);
        if (!role) return message.channel.sendErrorMessage(`Je n'ai pas trouver ce role.`);
        if (!settings.modRoles.includes(role.id)) return message.channel.sendErrorMessage(`Ce role n'est pas modérateur.`);
        const index = settings.modRoles.indexOf(role.id);
        settings.modRoles.splice(index, 1);
        await settings.save();
        return message.channel.sendSuccessMessage(`Le role ${role.name} n'est plus modérateur.`);
    }
