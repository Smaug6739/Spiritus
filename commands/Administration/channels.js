...
module.exports.run = async(client, message, args,settings) => {
    ...
    if(args[0].toLowerCase() === 'clone'){
        if(!message.member.hasPermission('MANAGE_CHANNELS'))return message.channel.send(`${client.config.emojis.error}Vous devez avoir la permission de gérer les channels pour utiliser cette commande.`);
        if(!message.guild.me.hasPermission('MANAGE_CHANNELS')) return message.channel.send(`${client.config.emojis.error}Je n'ai pas la permission de modifier des channel.`);
