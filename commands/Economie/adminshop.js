...
module.exports.run = async (client, message, args, settings, dbUser) => {
    ...
    if (args[0].toLowerCase() === 'add') {
        ...
        if (settings.shop.length > 14) return message.channel.sendErrorMessage(`Votre serveur à atteint le nombre maximal d'articles pouvant etre contenu dans le shop (15).`)
        const objetName = args[1]
        const objetPrix = parseInt(`${args[2]}`)
        const objetDescription = args.slice(3).join(' ')
        if (objetName.length > 30) return message.channel.sendErrorMessage(`Vous ne pouvez pas choisir un nom de plus de 30 caractères.`)
        if (objetDescription.length > 300) return message.channel.sendErrorMessage(` Vous ne pouvez pas choisir une description de plus de 300 caractères.`)
        if (isNaN(objetPrix) == true) return message.channel.sendErrorMessage(`Veuillez donner un nombre en prix à la place de \`${args[2]}\` .`);
        if (objetPrix > 999) return message.channel.sendErrorMessage(`Vous ne pouvez pas choisir un prix de plus de 999 ${client.config.emojis.coins}.`)
        if (settings.shop) {
            let existObj = settings.shop.find(e => e.name == objetName)
            if (existObj) return message.channel.sendErrorMessage(`Un objet avec ce nom existe déja dans la boutique merci de choisir un autre nom.`)
            let tableau = []
            ...
            message.channel.sendSuccessMessage(`L'objet \`${objetName}\` à bien été ajouter dans le shop.`)
        }
    }
    if (args[0].toLowerCase() === 'rem') {
        if (settings.shop) {
            if (args.length == 2 && args[1] == 'all') {
                ...
                settings.save();
                return message.channel.sendSuccessMessage(` Toutes les objets du shop ont bien été supprimés.`);
            } else {
                const title = args[1]
            ...
            return message.channel.sendErrorMessage(` Il n'y a aucun objets dans le shop de ce serveur.`)
        }
    }
}
module.exports.help = {
