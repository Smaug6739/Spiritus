...
module.exports.run = async (client, message, args) => {
  ...
  if(args[0].toLowerCase() === 'add'){
      ...
      const query = args[1]
          ...
          await settings.save();
          return message.channel.send(`${client.config.emojis.success}Ce mot est maintanant interdit ignorer.`);
      
  }
  if(args[0].toLowerCase() === 'rem'){
      if(!message.member.hasPermission('MANAGE_GUILD'))return message.channel.send(`${client.config.emojis.error}Vous devez avoir la permission de gérer le serveur pour utiliser cette commande.`);
      const query = args[1]
          if(!args[1]) return message.channel.send(`${clint.config.emojis.error}Merci d'indiquer le mot a supprimer de la liste.`)
